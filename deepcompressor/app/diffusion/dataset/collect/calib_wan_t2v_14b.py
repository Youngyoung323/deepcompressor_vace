# -*- coding: utf-8 -*-
"""Collect calibration dataset for Wan2.1-T2V-14B model.

Pipeline is created via WanVideoPipeline.from_pretrained() with T2V-14B DiT
weights (multi-shard ``diffusion_pytorch_model*.safetensors``), shared T5/VAE
as in ``ptq_wan_t2v._build_wan_t2v_14b_pipeline``.
...
Usage:
    python -m deepcompressor.app.diffusion.dataset.collect.calib_wan_t2v_14b
"""
...
def collect(
    ...
    for idx, sample in enumerate(
        tqdm(samples, desc="T2V-14B Calib", leave=False, dynamic_ncols=True),
        start=1,
    ):
        seq_id = f"{idx:06d}"
...
if __name__ == "__main__":
    # ─── Model (aligned with ptq_wan_t2v._build_wan_t2v_14b_pipeline) ───
    ...
    # ─── Generation parameters (match examples/diffusion/configs/model/wan2.1-t2v-14b.yaml) ───
    num_steps = 20
    height = 480
    width = 832
    num_frames = 81
    ...
    # ─── Output (path segment must match quant.calib.path in wan2.1-t2v-14b.yaml) ───
    output_root = "/data1/lyf/Lab/VACE/deepcompressor_vace/examples/diffusion/datasets"
    dataset_name = "Ditto-1M"
    pipeline_name = "Wan2.1-T2V-14B-20steps"
